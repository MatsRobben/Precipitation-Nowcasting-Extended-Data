...
from .layers.blocks import DBlock


class TemporalDiscriminator(torch.nn.Module):
    def __init__(
        self, input_channels: int = 12, num_layers: int = 3, conv_type: str = "standard"
    ):
        """
        ...
        self.d1 = DBlock(
            ...
            out_channels=internal_chn * input_channels,
            conv_type="3d",
            pre_activation=False,
        )
        self.d2 = DBlock(
            in_channels=internal_chn * input_channels,
            out_channels=2 * internal_chn * input_channels,
            conv_type="3d",
        )
        ...
        self.d_last = DBlock(
            in_channels=2 * internal_chn * input_channels,
            out_channels=2 * internal_chn * input_channels,
            downsample=False,
            conv_type=conv_type,
        )
    ...
    def random_crop(self, x):
        ...
        crop_size = height // 2  # Half of original height

        top = torch.randint(0, height - crop_size + 1, (batch_size,), device=x.device)
        left = torch.randint(0, width - crop_size + 1, (batch_size,), device=x.device)

        cropped_x = torch.stack(
            [
                x[b, :, :, t : t + crop_size, l : l + crop_size]
                for b, (t, l) in enumerate(zip(top, left))
            ],
            dim=0,
        )
...
if __name__ == "__main__":
    temp_dis = TemporalDiscriminator(input_channels=1, num_layers=3)
    spec_dis = SpatialDiscriminator(input_channels=1, num_timesteps=8, num_layers=4)

    dis = Discriminator(input_channels=1, num_spatial_frames=8)

    # Batch has to be greater then 2, because of batch norm.
    x = torch.rand((2, 22, 1, 256, 256))

    print("Output shape (TemporalDiscriminator):", temp_dis(x).shape)
    print("Output shape (SpatialDiscriminator):", spec_dis(x).shape)
    print("Output shape (Discriminator):", dis(x).shape)
